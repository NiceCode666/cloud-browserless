...
```
                              ┌── chromium-net (private bridge) ──┐
                              │                                   │
                ┌─────────────┴─────────────┐         ┌───────────┴────────────┐
浏览器 ─HTTPS─▶ │ cdp-gateway (nginx)       │         │ chromium               │
                │   :3443 TLS ──────────────┼────────▶│   :3000 selkies (UI)   │ ─▶ labwc 桌面
                │   :9443 HTTP + token ─────┼────────▶│   :9222 CDP            │       └─▶ Chromium
   CDP 客户端 ─▶│   :9080 HTTP (loopback)   │         │                        │
                └───────────────────────────┘         └────────────────────────┘
                  ↑ 唯一的对外入口                       ↑ 仅 chromium-net 可达
```

- **外部流量只经过 gateway 的 3 个端口**（宿主侧）：Web UI（HTTPS）、CDP（带 token）、SSH 隧道用的 loopback。
- gateway 与 chromium 通过私有 docker bridge `chromium-net` 通信，gateway 用 DNS 名 `chromium` 访问。
- chromium 自身**不映射任何宿主端口**，CDP 的 `9222` 只在 docker 内部可达，公网/宿主直接访问不了，安全靠 gateway 的 token 鉴权。
- 这种解耦让 chromium 能被任意重启 / 重建（`docker restart`、`docker compose recreate`、kill 后 docker 自动拉起、…）而 **gateway 不需要任何特殊处理就能自动恢复**：docker 内置 DNS 会重新解析 `chromium` 服务名到新容器的 IP。
...
- `/opt/cloud-browser/chromium-profile/` — **Chromium 用户档案**（cookies、登录态、扩展、书签）。备份/迁移只需打包此目录
- `/opt/cloud-browser/chromium-config/` — 桌面环境、proot 装的 App、下载文件等其他持久数据
...
**Q: 镜像拉不动（国内服务器）？**
A: 镜像来自 Docker Hub，通常受益于国内 mirror。服务器 `/etc/docker/daemon.json` 配了 `registry-mirrors` 基本无痛。

**Q: 为什么 `docker-compose.yml` 把镜像钉死在 `linuxserver/chromium:cc0cbc1c-ls25` 而不是 `:latest`？**
A: linuxserver 在 2026-03-31 把 selkies 默认渲染切到了 Wayland，新版 `pixelflux` Rust 库在 Docker Desktop / LinuxKit 内核上初始化共享内存时直接 `EOPNOTSUPP` panic，启动循环。`cc0cbc1c-ls25` 是 Wayland 切默认前的最后一个稳定 tag。等上游修了 pixelflux 再升 tag。

**Q: chromium 重启后 gateway 会不会卡住？**
A: 不会。架构层面 gateway 与 chromium 走私有 docker bridge，gateway 用 DNS 名 `chromium` 访问。任何方式重建/重启 chromium，docker 内置 DNS 会把 `chromium` 解析到新 IP，gateway nginx 第一个请求重新建立 TCP 连接即可恢复，无需手动干预。

> 历史背景：早期版本用 `network_mode: service:chromium` 共享 netns，chromium 重建会让 gateway 绑在已死的 netns 上、看似 Up 实则不监听任何端口。`./run-local.sh restart` 当时必须 force-recreate 两个容器，且任何 `docker restart cloud-chromium` 都会让 gateway 失能。改造后这个故障模式彻底消失。
...
- `gateway/certs/` — 自签证书
- `chromium-profile/` — Chromium 用户档案（绑定挂载到容器 `/profile`，被 `--user-data-dir` 钉死）
- `chromium-config/` — 桌面环境其他持久数据
